...
from pytorch3d.renderer import (FoVPerspectiveCameras, VolumeRenderer, NDCGridRaysampler, EmissionAbsorptionRaymarcher)
...
class VolumeModel(torch.nn.Module):
    def __init__(self, renderer, volume_size=[64, 64, 64], voxel_size=0.1):
        super().__init__()
...
def huber(x, y, scaling=0.1):
    diff_eq = (x - y) ** 2
    loss = ((1 + diff_eq / (scaling ** 2)).clamp(1e-4).sqrt() - 1)
    return loss
...
volume_size = 128
volume_model = VolumeModel(renderer=renderer, volume_size=[volume_size] * 3,
...
for iteration in range(n_iter):
    ...
    rendered_images, rendered_silhouettes = volume_model(batch_cameras).split([3, 1], dim=3)
...
with torch.no_grad():
    rotating_volume_frames = generate_rotating_volume(volume_model, n_frames=28)
